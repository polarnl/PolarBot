...
export default {
    ...
    async openTicket(interaction, client) {
    ...
    async addUser(interaction) {
        ...
        try {
            ...
            // Update database
            await ticketsCollection.updateOne({ _id: ticket._id }, { $addToSet: { participants: targetUser.id } });
            const embed = new EmbedBuilder()
                .setColor(0x00FF00)
                .setTitle("➕ User Added")
    ...
    async removeUser(interaction) {
        ...
        try {
            ...
            // Update database
            await ticketsCollection.updateOne({ _id: ticket._id }, { $pull: { participants: targetUser.id } });
            const embed = new EmbedBuilder()
                .setColor(0xFF0000)
